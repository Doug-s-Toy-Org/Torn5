			...
			set
			{
				...
				if (holder != null)
				{
					if (numericPyramidTeams.Value == 1 && numericPyramidDesiredTeamsPerGame.Value == 1)
					{
						...
						if (holder.League.TeamCount() > 3)
						{
							numericPyramidTeams.Value = holder.League.TeamCount();
							var games = holder.League.Games();
							if (games.Any())
								numericPyramidDesiredTeamsPerGame.Value = (int)Math.Round(games.Average(g => g.Teams.Count));
							else
								numericPyramidDesiredTeamsPerGame.Value = (int)Math.Round(Math.Sqrt(holder.League.TeamCount()));
						}
