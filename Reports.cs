...
namespace Torn.Report
{
	...
	public static class Reports
	{
		...
		public static ZoomReport GamesGrid(League league, List<Game> games, ReportTemplate rt)
		{
			...
			if (rt.ReportType == ReportType.GameGrid)
			{
				...
				if (rt.Drops != null && (rt.Drops.DropWorst(100) > 0 || rt.Drops.DropBest(100) > 0))
					report.AddColumn(new ZColumn("Dropped", ZAlignment.Integer) { Rotate = true });
			}
		...
		public static ZoomReport GamesGridCondensed(League league, List<Game> games, ReportTemplate rt)
		{
			...
			foreach (var tc in titleCount)
				for (int i = 0; i < tc.Value; i++)
				{
					...
					report.AddColumn(new ZColumn(league.IsPoints() ? "Points" : "Rank", ZAlignment.Right) { GroupHeading = tc.Key, Rotate = true });
				}
		...
		public static ZoomReport GamesList(League league, List<Game> games, ReportTemplate rt)
		{
			...
			{
				report.AddColumn(new ZColumn("Team", ZAlignment.Left));
				report.AddColumn(new ZColumn("Score", ZAlignment.Right) { Rotate = league.IsPoints() });
				if (hasHits)
					report.AddColumn(new ZColumn("Hits", ZAlignment.Right) { Rotate = league.IsPoints() });
				if (league.IsPoints())  // there are victory points for this league
					report.AddColumn(new ZColumn("Points", ZAlignment.Right) { Rotate = true });
				else
		...
		public static ZoomReport DetailedGamesList(League league, List<Game> games, ReportTemplate rt)
		{
			...
			{
				if (row[col].Tag is double maxScore)
		...
		public static ZoomReport OneTeam(League league, bool includeSecret, LeagueTeam team, string path, DateTime from, DateTime to, bool description)
		{
			...
			if (league.IsPoints())
				report.AddColumn(new ZColumn("Points", ZAlignment.Float, "Score") { Rotate = true });
			else
				report.AddColumn(new ZColumn("Rank", ZAlignment.Integer) { Rotate = true });

			report.AddColumns("Score again,Tags +,Tags -,Tag Ratio,Score Ratio,TR\u00D7SR,Destroys,Conceded,Ratio,Denies,Got Denied,Yellow Card,Red Card,Played Against",
			...
			foreach (GameTeam gameTeam in played.OrderBy(gt => league.Game(gt).Time))
				if (from < league.Game(gameTeam).Time && league.Game(gameTeam).Time < to)
				{
					...
					gameRow.Add(new ZCell(gameTeam.Score, ChartType.Bar, "N0", gameTeam.Colour.ToColor()));  // team Total Score

					if (league.IsPoints())
					{
						if (game.IsPoints())
							gameRow.Add(new ZCell(gameTeam.Points, ChartType.None, "N0", gameTeam.Colour.ToColor()));  // Points
						else
							gameRow.Add(new ZCell());  // Points
					}
					else
						gameRow.Add(new ZCell(game.Teams.IndexOf(gameTeam) + 1));  // Rank
					...
					if (game.Teams.Count < 10)  // Played Against
					{
						var text = new StringBuilder();
			...
			report.RemoveColumn(report.Columns.IndexOf(report.Columns.Find(c => c.Text == "Score again")));  // "Score again" is a column added to keep FillDetails() happy. It contains duplicate data.
			report.RemoveZeroColumns();
			return report;
		}
		...
		public static ZoomReport SoloLadder(League league, List<Game> games, ReportTemplate rt)
		{
			...
			foreach (var pt in playerTeams)
			{
				...
				if (thisPlayersGames.Count() >= atLeastN && player.Name != null)
				{
					...
					row.Add(DataCell(played.Select(x => (double)x.Rank).ToList(), rt.Drops, chartType, "N1"));  // Av rank
					...
					row.Add(DataCell(played.Select(x => (double)x.YellowCards).ToList(), rt.Drops, ChartType.Bar, "G2"));
					row.Add(DataCell(played.Select(x => (double)x.RedCards).ToList(), rt.Drops, ChartType.Bar, "G2"));
					row.Add(new ZCell(played.Count(x => x.IsEliminated), ChartType.Bar, "N0"));

					row.Add(new ZCell(played.Count(), ChartType.None, "N0", default));  // Games
					...
					else
					{
						...
						if (countAfterDrops < played.Count)
							row.Add(new ZCell(played.Count - countAfterDrops, ChartType.None, "N0", default));  // games dropped
						else
							row.Add(new ZCell());  // games dropped
					}
		...
		public static ZoomReport TermReport(League league, List<Game> games, ReportTemplate rt)
		{
			...
			foreach (var pt in playerTeams)
			{
				...
				if (thisPlayersGames.Count() >= atLeastN && player.Name != null)
				{
					...
					row.Add(TotalDataCell(played.Select(x => (double)(x.TermRecords?.FindAll(r => r.Type == TermType.Yellow)?.Count() ?? 0)).ToList(), rt.Drops, ChartType.Bar, "N0"));
					row.Add(TotalDataCell(played.Select(x => (double)(x.TermRecords?.FindAll(r => r.Type == TermType.Verbal)?.Count() ?? 0)).ToList(), rt.Drops, ChartType.Bar, "N0"));
					row.Add(TotalDataCell(played.Select(x => (double)(x.TermRecords?.FindAll(r => r.Type == TermType.Other)?.Count() ?? 0)).ToList(), rt.Drops, ChartType.Bar, "N0"));
...
Tiny numbers at the bottom of the bottom row show the minimum, bin size, and maximum of each histogram.";
		...
		{
			var dataCell = new ZCell(0, chartType, numberFormat, default);
		...
		static ZoomReport FinishReport(ZoomReport report, IEnumerable<Game> games, ReportTemplate rt)
		{
			...
			if (!games.Any())
			{
				if (rt.From == null && rt.To == null)
					...
					report.Description += " No games were found " + FromTo(games, rt.From, rt.To) + ".";
			}
	...
	class PyramidComparer : IComparer<ZRow>
	{
		...
		void CalcRoundScores(ZRow row, int lastCol, string group, out double averageScore, out double averagePoints)
		{
			...
			for (int i = 0; i <= lastCol; i++)
				...
				{
					if (Columns[i].Text == "Points" || Columns[i].Text == "Rank")
		...
		int LastGame(ZRow row, int lastCol)
		{
			...
			while (col > 0 && (col >= Columns.Count || Columns[col].Text == "Average" || Columns[col].Text == "Points"))
				col--;
